...
class GUI:
    ...
    def __init__(self, master) -> None:
        ...
        #input frame
        frame_in = frame_out = self._create_labeled_frame(self.master, "Input", "top")
    ...
    def _input_handler(self, event):
        ...
        result = self.backend.write(chars)
        if result == True:
    ...
    def _output_handler(self, message):
        ...
        view = self.text_output.yview()
        self.text_output.delete(1.0, END)
        self.text_output.insert('end-1c', message.decode('utf-8'))
        self.text_output.config(state='disabled')
        self.text_output.yview_moveto(str(view[0]))