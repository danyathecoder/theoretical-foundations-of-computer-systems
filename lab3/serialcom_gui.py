...
class GUI:
    ...
    def __init__(self, master) -> None:
        ...
        self.text_debug = scrolledtext.ScrolledText(frame_debug_text, height=self.scrolled_text_height, state='disabled')
        self.text_debug.pack(expand=1, fill='x', padx=20)
        self.text_debug.tag_config('red', foreground='red')
        self.text_debug.tag_config('bold', font=('Helvetica', 8, 'bold'))

        self.backend = Logic(self._debug_handler, self._output_handler)
    ...
    def _debug_handler(self, string, color='black'):
        self.text_debug.config(state='normal')
        self.text_debug.insert('end-1c', string)
        self.text_debug.see(END)
        if color == 'red':
            self.text_debug.tag_add('red', 'end-' + str(len(string) + 1) + 'c', 'end-1c')
        if color == 'bold':
            self.text_debug.tag_add('bold', 'end-' + str(len(string) + 1) + 'c', 'end-1c')
